...
#### Added - Level Full Mastery & Notifications Improvement

- **Achievements**: Added `levelFullMastery` achievement type to track completion of all 6 difficulties in a single level.
- **Seeder**: Updated `seed.ts` to include the first level mastery achievement.
- **Notifications**:
  - Fixed route ordering for `mark-all-read` endpoint.
  - Improved `markAllAsRead` controller performance using optimized batch updates.
  - Refined frontend notification indicator to separate persistent notifications from achievement alerts.
