...
export default {
    ...
    if (data.reward) {
      try {
        ...
        if (typeof data.reward === "string") {
          ...
          data.reward.connect &&
          data.reward.connect.length > 0
        ) {
          // Handle connect syntax { connect: [{ id: ... }] } or { connect: [id] }
          const connectId =
            data.reward.connect[0]?.id || data.reward.connect[0];
          if (connectId) {
            reward = await strapi.db.query("api::reward.reward").findOne({
        ...
        strapi.log.error(
          "[UserReward Lifecycle] Error initializing claim fields:",
          error,
        );
      }
    }
  ...
  async afterCreate(event) {
    ...
    try {
      ...
      strapi.log.error(
        "[UserReward Lifecycle] Error in afterCreate notification:",
        error,
      );
    }
  },
  ...
  async afterUpdate(event) {
    ...
    if (params.data.rewardStatus && result) {
      try {
        ...
        strapi.log.error(
          "[UserReward Lifecycle] Error in afterUpdate notification:",
          error,
        );
      }
    }
