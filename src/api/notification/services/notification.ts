    ...
    try {
      const messages = {
        pending: {
          ...
          message: 'ha sido recibida. Por favor sube los documentos requeridos para continuar.',
          priority: 'HIGH',
        },
        processing: {
        ...
        delivered: {
          ...
          message: claim.trackingNumber
            ? `ha sido enviado. Tracking: ${claim.trackingNumber}`
            : 'ha sido enviado',
          priority: 'HIGH',
        },
        rejected: {
      ...
      if (!userId) {
        strapi.log.warn(`Cannot create notification: no user for claim ${claim.id}`);
        return null;
      }
