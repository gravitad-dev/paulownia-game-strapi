...
export default factories.createCoreController('api::reward.reward', ({ strapi }) => ({
  ...
  async spin(ctx) {
    ...
    const allRewards = await strapi.entityService.findMany('api::reward.reward', {
      filters: {
        isActive: true,
        quantity: { $gt: 0 },
      },
    ...
    const userRewards = await strapi.entityService.findMany('api::user-reward.user-reward', {
      filters: {
    ...
    if (availableRewards.length === 0) {
      return ctx.badRequest('No rewards available (all unique rewards obtained)', {
    ...
    await strapi.entityService.update('api::player-stat.player-stat', playerStat.id, {
      data: {
    ...
    await strapi.entityService.update('api::reward.reward', selectedReward.id, {
      data: {
    ...
    if (selectedReward.typeReward === 'currency') {
      ...
      if (isCoin) {
        updateData.coins = (playerStat.coins || 0) + (selectedReward.value || 0);
        updateData.coinsEarned = (playerStat.coinsEarned || 0) + (selectedReward.value || 0);
      } else {
        updateData.tickets = (playerStat.tickets || 0) + (selectedReward.value || 0);
        updateData.ticketsEarned = (playerStat.ticketsEarned || 0) + (selectedReward.value || 0);
      }
      ...
      userReward = await strapi.entityService.create('api::user-reward.user-reward', {
        data: {
          users_permissions_user: user.id,
    ...
    await strapi.entityService.create('api::roulette-history.roulette-history', {
      data: {
        users_permissions_user: user.id,
        reward: selectedReward.id,
        timestamp: now,
